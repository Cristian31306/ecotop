...
const addCard = () => {
    form.content.push({ text: '', image: null });
};
...
const submit = () => {
    // Al usar archivos, Inertia automáticamente usa FormData
    form.post(route('admin.ecosystems.store'));
};
</script>

<template>
    <Head title="Crear Ecosistema" />

    <AuthenticatedLayout>
        <div class="py-12 max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-6 flex justify-between items-center">
                <h2 class="text-2xl font-bold text-gray-800">Añadir Nuevo Ecosistema</h2>
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="flex justify-end pt-4 border-t border-gray-200">
                        <button type="submit" :disabled="form.processing" class="btn-primary" :class="{'opacity-50': form.processing}">
                            {{ form.processing ? 'Guardando...' : 'Guardar Ecosistema' }}
                        </button>
                    </div>
                </form>
            </div>
